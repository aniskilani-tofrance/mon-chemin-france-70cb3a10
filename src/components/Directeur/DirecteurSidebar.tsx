...
interface NavItem {
  ...
  url: string;
  icon: any;
  badgeKey?: "totalLearners" | "totalFormateurs" | "hotLeads";
  end?: boolean;
}
...
const groups: { label: string; items: NavItem[] }[] = [
  {
    label: "Vue",
    items: [{ title: "Tableau de bord", url: "/directeur", icon: LayoutDashboard, end: true }],
  },
  {
    label: "Pilotage",
    items: [
      {
        title: "Apprenants",
        url: "/directeur/apprenants",
        icon: Users,
        badgeKey: "totalLearners",
      },
      {
        title: "Formateurs",
        url: "/directeur/formateurs",
        icon: UserCog,
        badgeKey: "totalFormateurs",
      },
      { title: "Performance", url: "/directeur/performance", icon: BarChart3 },
    ],
  },
  {
    label: "Acquisition",
...
export function DirecteurSidebar() {
  const { state } = useSidebar();
  const collapsed = state === "collapsed";
  const { user, signOut } = useAuth();
  const navigate = useNavigate();
  const stats = useDirecteurStats();
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data } = await supabase
        ...
        .eq("user_id", user.id)
        .maybeSingle();
      setDisplayName(data?.full_name || data?.email || user.email || "Directeur");
    };
    load();
  }, [user]);
  ...
  const initials = (displayName || "D").charAt(0).toUpperCase();
        ...
        <div className="flex items-center gap-2 px-2 py-2">
          ...
          {!collapsed && (
            <div className="flex flex-col leading-tight">
              ...
              <span className="text-[11px] text-muted-foreground">Espace directeur</span>
            </div>
          )}
        </div>
      </SidebarHeader>
                        ...
                        >
                          ...
                          {!collapsed && (
                            <>
                              ...
                              {showBadge && (
                                <Badge
                                  variant={item.badgeKey === "hotLeads" ? "destructive" : "secondary"}
        ...
        <div className={`flex items-center gap-2 px-2 py-2 ${collapsed ? "justify-center" : ""}`}>
          ...
          {!collapsed && (
            <div className="flex-1 min-w-0">
              ...
              <div className="text-[10px] text-muted-foreground">Directeur</div>
            </div>
          )}
        </div>
        <Button
